...
void am_deactivate(struct am_device_notification_callback_info *info, void *foo) {
	...
	if (! AMDeviceConnect(apple_device)) {
		...
		if (AMDeviceIsPaired(apple_device)) {
			...
			if (! AMDeviceValidatePairing(apple_device)) {
				...
				if (! AMDeviceStartSession(apple_device)) {
					...
					if (! AMDeviceDeactivate(apple_device)) {
						
						printf("[AMDevice] Deactivated device.\r\n");
						exit(1);
...
void am_recoverydisconnect(struct am_recovery_device *rdev) {
	//not required (however prevents a bus error).
}

int main (int argc, const char * argv[]) {
	
	printf("\r\nMule (deactivate) - Copyright GreySyntax NSPwn.com 2010\r\n\r\n");
	
	struct am_device_notification *notif;
	AMDeviceNotificationSubscribe((am_device_notification_callback) am_deactivate, 0, 0, 0, &notif);
